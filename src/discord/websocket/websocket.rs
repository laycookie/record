...
use crate::discord::{
    rest_api::discord_endpoints::DISCORD_GATEWAY, websocket::websocketconfig::discord_intents,
};
...
pub async fn websocket_init(token: &str) {
    ...
    let Payload {
        identify,
        heart_beat,
    } = Payload::build(discord_intents::DIRECT_MESSAGES, token);
    write
    ...
    tokio::spawn(async move {
        ...
        write
            .send(Message::Text(heart_beat.to_string()))
            .await
            .expect("failed to send heartbeat to a server");
        loop {
